...
<div class="menuBoxInner biblioIcon">
  ...
  <div class="sub_section col-md-12">
	  <div class="btn-group">
	  ...
	  <form role="form" class="form-inline" name="search" action="<?php echo MWB; ?>bibliography/index.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
		  <input type="text" name="keywords" id="keywords" class="form-control" size="30" />
		  <select name="field" class="form-control"><option value="0"><?php echo __('All Fields'); ?></option><option value="title"><?php echo __('Title/Series Title'); ?> </option><option value="subject"><?php echo __('Topics'); ?></option><option value="author"><?php echo __('Authors'); ?></option><option value="isbn"><?php echo __('ISBN/ISSN'); ?></option><option value="publisher"><?php echo __('Publisher'); ?></option></select>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
  ...
  $form->table_attr = 'align="center" id="dataList" class="table" cellpadding="5" cellspacing="0"';
  ...
  /* Form Element(s) */
  // biblio title
  $form->addTextField('textarea', 'title', __('Title').'*', $rec_d['title'], 'class="form-control" rows="1" style="width: 100%; overflow: auto;"',
  ...
  $form->addTextField('text', 'sor', __('Statement of Responsibility'), $rec_d['sor'], 'class="form-control" style="width: 40%;"', __('Main source of information to show who has written, composed, illustrated, or in other ways contributed to the existence of the item.'));
  // end of modification

  // biblio edition
  $form->addTextField('text', 'edition', __('Edition'), $rec_d['edition'], 'class="form-control" style="width: 40%;"', __('A version of publication having substantial changes or additions.'));
  // biblio specific detail info/area
  $form->addTextField('textarea', 'specDetailInfo', __('Specific Detail Info'), $rec_d['spec_detail_info'], 'class="form-control" rows="2" style="width: 100%"', __('explain more details about an item e.g. scale within a map, running time in a movie dvd.'));
  // biblio item batch add
  $str_input = __('Pattern').': <input type="text" class="medium_input" name="itemCodePattern" value="'.$sysconf['batch_item_code_pattern'].'" /> &nbsp;';
  ...
  $str_input .= __('Collection Type').': '.simbio_form_element::selectList('collTypeID', $coll_type_options, '', 'style="width: 150px;"');;
  ...
  // biblio series title
  $form->addTextField('textarea', 'seriesTitle', __('Series Title'), $rec_d['series_title'], 'class="form-control" rows="1" style="width: 100%;"');
  ...
  // biblio note
  $form->addTextField('textarea', 'notes', __('Abstract/Notes'), $rec_d['notes'], 'class="form-control" style="width: 100%;" rows="2"', __('Insert here any abstract or notes from the publication.'));
  // biblio cover image
  $str_input = '';
  if ($rec_d['image']) {
    $str_input = '<div id="imageFilename"><a href="'.SWB.'images/docs/'.$rec_d['image'].'" class="openPopUp notAJAX"><strong>'.$rec_d['image'].'</strong></a> <a class="btn btn-danger btn-xs" href="'.MWB.'bibliography/index.php" postdata="removeImage=true&bimg='.$itemID.'&img='.$rec_d['image'].'" loadcontainer="imageFilename" class="makeHidden removeImage">'.__('REMOVE IMAGE').'</a></div>';
  }
  ...
  if ($sysconf['scanner'] !== false) {
    ...
    $str_input .= '<textarea class="form-control" id="base64picstring" name="base64picstring" style="display: none;"></textarea>';
  ...
  echo '<div class="infoBox">'
    .'<div style="float: none; width: 80%;">'.__('You are going to edit biblio data').' : <b>'.$rec_d['title'].'</b>  <br />'.__('Last Updated').'&nbsp;'. $rec_d['last_update'].'</div>'; //mfc
    if ($rec_d['image']) {
  ...
  $datagrid->table_attr = 'align="center" id="dataList" class="table table-hover" cellpadding="5" cellspacing="0"';
