...
class serial
{
    ...
    public function viewKardexes()
    {
        ...
        // button
        $_button_grp = '<div style="padding: 3px; background: #999999;"><input type="submit" class="btn btn-primary" name="saveKardexes" value="'.__('Save').'" /></div>';

        // header
        echo '<div style="padding: 5px; background: #CCCCCC;">'.__('Kardex Detail for subscription').' <strong>'.$this->serial_period.'</strong></div>';
        if ($_can_read AND $_can_write) {
            echo '<form role="form" method="post" name="kardexListForm" id="kardexListForm" action="'.$_SERVER['PHP_SELF'].'">';
            echo $_button_grp;
        }
        ...
        if ($_can_read AND $_can_write) {
            ...
            echo '</form>';
            // hidden form
            echo '<form role="form" name="hiddenActionForm" method="post" action="'.$_SERVER['PHP_SELF'].'"><input type="hidden" name="serialID" value="0" /><input type="hidden" name="remove" value="0" /></form>';
        }
        /* main content end */
