...
<div class="menuBoxInner memberIcon">
    ...
    <form role="form" name="search" action="<?php echo MWB; ?>membership/index.php" id="search" method="get" style="display: inline;"><?php echo __('Member Search'); ?> :
	    <input type="text" name="keywords" size="30" /><?php if (isset($_GET['expire'])) { echo '<input type="hidden" name="expire" value="true" />'; } ?>
	    <input type="submit" id="doSearch" value="<?php echo __('Search'); ?>" class="btn btn-primary" />
	</form>
	</div>
</div>
</fieldset>
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    $form->table_attr = 'align="center" id="dataList" class="table table-hover" cellpadding="5" cellspacing="0"';
    ...
    if ($rec_q->num_rows > 0) {
        ...
        $form->submit_button_attr = 'name="saveData" value="'.__('Update').'" class="btn btn-primary"';
    }
    ...
    if ((isset($_GET['keywords']) AND $_GET['keywords']) OR isset($_GET['expire'])) {
        ...
        if (isset($_GET['expire'])) {
            ...
            echo '<div><input type="button" value="'.__('Extend Selected Member(s)').'" onclick="javascript: if (confirm(\''.__('Are you sure to EXTEND membership for selected members?').'\')) { $(\'#mainContent\').simbioAJAX(\''.MWB.'membership/index.php?expire=1\', { method: \'post\', addData: $(\'#memberList\').serialize() + \'&batchExtend=true\' } ); }" class="btn btn-primary" /></div>';
